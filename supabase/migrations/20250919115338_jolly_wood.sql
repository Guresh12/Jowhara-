/*
  # Complete Jowhara Collection Database Schema
  ...
  2. Security
    ...
    - Admin access via Supabase Auth with app_metadata.role = 'admin'
...
INSERT INTO products (name, slug, sku, category_id, brand_id, price, description, ingredients, usage_instructions, images, stock, featured) VALUES
  ...
  ('Fresh Citrus Body Spray', 'fresh-citrus-body-spray', 'JW-BODY-001', 
   (SELECT id FROM categories WHERE slug = 'body-sprays'), 
   (SELECT id FROM brands WHERE slug = 'chanel'), 
   1200, 'Refreshing citrus body spray', 'Citrus extracts, Natural oils', 'Spray all over body after shower', 
   '["https://images.pexels.com/photos/4465124/pexels-photo-4465124.jpeg?auto=compress&cs=tinysrgb&w=800"]', 40, false),
  
  ('Lavender Room Spray', 'lavender-room-spray', 'JW-AIR-001', 
   (SELECT id FROM categories WHERE slug = 'air-fresheners'), 
   (SELECT id FROM brands WHERE slug = 'dior'), 
   800, 'Calming lavender air freshener', 'Lavender essential oil, Natural extracts', 'Spray in room as needed', 
   '["https://images.pexels.com/photos/4210374/pexels-photo-4210374.jpeg?auto=compress&cs=tinysrgb&w=800"]', 50, false)
